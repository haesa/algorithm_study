...
directions = {}
for _ in range(L):
    t, direct = input().strip().split()
    directions[int(t)] = direct
...
while True:
    ...
    # 벽이거나 뱀 몸통이 있는 경우 게임 종료
    if not (0 <= cur_r < N and 0 <= cur_c < N) or board[cur_r][cur_c] == 1:
        print(sec)
        break
    ...
    # 방향 전환
    if sec in directions:
        if directions[sec] == "L":
            i = (i + 3) % 4
        elif directions[sec] == "D":
            i = (i + 1) % 4
